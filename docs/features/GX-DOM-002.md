...
- The authoritative live snapshot remains complete. Depth selection is a frontend projection only and does not alter the Caido query, scope, subscriptions, pending-change tracking, or stored data.
...
| GX-DOM-203 | Keep the complete live snapshot and ingestion pipeline independent from the selected visual depth. | Projection runs after `useDomainGraph` | Architecture review and installed manual-sync smoke test. |
| GX-DOM-204 | Preserve valid depth selection across syncs and reset it to `All` on scope change. | `useDomainGraphDepth` | Composable unit tests and installed sync smoke test. |
